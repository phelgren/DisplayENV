...
(function (win, doc) {
	...
	mnemBox+= '</span>';
	mnemBox+= '<span class="mnem-box">';
	mnemBox += '<input placeholder="server name" class="sip-input sn-index-input" type="text" maxlength="35" data-prev_val="##server##" value="##server##" />';
	mnemBox+= '</span>';
	...
	function isValidSet (ip, mnem, color, server) {
		return ((!! ip) && (ip.match(valid_ip) || ip.match(valid_cidr) && (server && server.length && (server.length > 0)) || (color && color.length && (color.length > 0)) || (mnem && mnem.length && (mnem.length > 0))));
	}

	function addMnem (ip, mnem, color, server) {
		if (! mnems[ip]) {
			mnems[ip] = {};
		}
		mnems[ip].mnem = mnem;
		mnems[ip].color = (!! color) ? color : '';
		mnems[ip].server = (!! server) ? server : '';
		update_mnems();
	}
	...
	function change_mnem () {
		...
		var my_gparent = this.parentNode.parentNode,
			...
			my_sn_input = my_gparent.getElementsByClassName('sn-index-input')[0],
		...
		if (isValidSet(my_ip_input.value, my_mnem_input.value, myColorInput.value, my_sn_input.value) && ((my_ip_input.value !== my_ip_input.dataset.prev) || (my_mnem_input.value !== my_mnem_input.dataset.prev) || (myColorInput.value !== myColorInput.dataset.prev) || (my_sn_input.value !== my_sn_input.dataset.prev))) {
			show_message(my_gparent, '');
			if (my_ip_input.dataset.prev) {
				deleteMnem(my_ip_input.dataset.prev);
			}
			addMnem(my_ip_input.value, my_mnem_input.value, myColorInput.value, my_sn_input.value);
			...
			my_sn_input.dataset.prev = my_sn_input.value;
			show_message(my_gparent, 'saved (' + my_mnem_input.value + ' : ' + my_ip_input.value + ' : ' + myColorInput.value +  ' : ' + my_sn_input.value +')');
		}
	}
